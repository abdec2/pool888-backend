...
module.exports = {
  calculate: async (ctx, next) => {
    try {
      ...
      const contract = new ethers.Contract('0xc0274E11cca5561c15060a59D5999E15C14b9908', ContractInterface, provider);
      const reward = await contract.pendingToken(1, hWalletAddress);

      let profit = parseFloat(ethers.utils.formatUnits(reward, '20'));
      let commission = null;
      ...
      if (referral) {
        commission = await Promise.all(referral.map(async item => {
          try {
            ...
            const childProfit = parseFloat(ethers.utils.formatUnits(childReward, '20'));
